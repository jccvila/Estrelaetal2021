...
''' Extension to Cobras model class to make models ready for CAFBA
	...
	This code is only compatabile with models using Biggs Notation.'''
import cobra
import sys
import numpy as np
import os
...
class CAFBA_Model(cobra.Model):
	...
	def set_minimal_media(self,ions=[],auto_ions=True,ignore =[]):
	...
	def set_full_media(self,default_vmax=1000,ions =[], auto_ions=True,ignore=[]):
	...
	def calculate_cost(self,metabolite,grate):
		...
		with self as m:
			...
			free_cafba = m.reactions.cafba_rxn.upper_bound- sol2.objective_value
			if free_cafba ==0:
	...
	def transporters_list(self,ignore_list = ['DMSOR1e', 'DMSOR2e', 'GLCDe', 'TMAOR1e', 'TMAOR2e'],auto_ignore =True):
		'''take models and returns lits of transporters it. At the moment
		it ignore sprotons when deciding if a reaction is a transporters
		ignore protons when deciding if reaction is a transporters
		ignore list is list of reacts that are not transporters, but are instead involved in oxidative phosphorylation / intracellular metabolism.'''	
		def is_transport(react):
			mets = {key:value for key, value in react.metabolites.items() if str(key)[:-1] !='h_' and str(key)[:-1] !='na1_' and str(key) != 'CAFBA_met'}
			...
			test = 'e' in prod_mets and  ('c' in con_mets or 'p' in con_mets)
			#does metabolite go cytoplasm to periplasm
			test2 = 'p' in prod_mets  and  ('c' in con_mets)
			#does metabolite go from extracellular compt to cytoplasm or periplasm
			test3 = 'e' in con_mets and  ('c' in prod_mets or 'p' in prod_mets)
			#does metabolite go periplasm to cytoplasm
			test4 = 'p' in con_mets  and  ('c' in prod_mets)
			test5 = len(mets) == 0
			return test or test2 or test3 or test4 or test5
		rxn = [y.id for y in self.reactions if len(y.metabolites) != 1 and is_transport(y) and y.objective_coefficient ==0]
		if auto_ignore ==True:
	...
	def calibrate_e_sector(self,csource = 'glc__D_e',grate =0.85,thresh=50,starting_cost = 0.00083):
		...
		transporters = self.transporters_list() 
		cost = starting_cost
		self.set_minimal_media(ignore=[csource])
		self.slim_optimize()
		with self as m:
			...
			while np.abs(f - grate) > 1e-6:
				...
				for t in e_sector_reactions:
					t.add_metabolites({um : -cost},combine=False)
					if counter>thresh:
						print('Failed to converge for ' + 'glc__D_e')
						break
					else:
						f = m.slim_optimize()
			print('w_e = ' + str(cost))
	...
	def quick_cafbafy(self,wr=0.169,phi_q=0.45,grate =1):
		self.reassign_compartments()
		self.irreversabilize()
		self.prune_model()
		self.CAFBAfy(wr = wr,phiQ_0 =phi_q)
		self.set_full_media()
		self.calibrate_e_sector(grate=grate)
